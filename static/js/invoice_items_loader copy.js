  ...
  fetch(`/invoices/get_invoice_items/${invoiceId}/`)
    ...
    .then(data => {
      ...
      data.items.forEach((item, index) => {
        ...
        // ✅ ensure DELETE exists but disable it in readonly mode
      ...
      document.getElementById("vat-amount").textContent = parseFloat(data.vat_amount).toFixed(2) + " €";
      document.getElementById("grand-total").textContent = parseFloat(data.grand_total).toFixed(2) + " €";

      lockInvoiceItems(true);
    })
    .catch(err => console.error("loadInvoiceItems error:", err));
}
